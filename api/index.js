...
const app = express();
// Using a wide open CORS for Vercel deployment simplicity. Restrict this later.
...
// NOTE: We only exit the process if we are running locally (not on Vercel)
...
async function createIndexes() {
  if (!db) return; // Ensure DB is connected before trying to create indexes
  try {
...
async function connectToDatabase() {
  // Vercel optimization: Reuse client connection across serverless cold starts
  if (db) {
  ...
  try {
    ...
    while (retries > 0) {
      try {
        ...
        if (retries === 0) {
          console.error('All connection retries exhausted. Throwing error.');
    ...
    // Create indexes for better performance (Only run on connection, not every request)
    await createIndexes();
    return db;
    ...
    if (err.message.includes('TLSV1_ALERT_INTERNAL_ERROR') || err.message.includes('SSL') || err.message.includes('ENOTFOUND') || err.message.includes('ETIMEDOUT') || err.message.includes('ECONNREFUSED')) {
      ...
      console.error('1. **MongoDB Atlas IP Whitelist:** Ensure the IP range 0.0.0.0/0 is temporarily added to your Atlas project\'s Network Access List for deployment testing.');
      console.error('2. **Connection String:** Verify your MONGODB_URI is set correctly in Vercel\'s Environment Variables.');
      console.error('');
    }
...
function withDatabaseErrorHandling(operation) {
  return async (req, res, next) => {
    try {
      // Critical: Ensure DB connection is made before any operation
      if (!db) {
...
function authMiddleware(req, res, next) {
  // Check for required environment variables only once the endpoint is hit
  if (!JWT_SECRET) {
  ...
  if (parts.length !== 2 || parts[0].toLowerCase() !== 'bearer') return res.status(401).json({ error: 'Invalid Authorization header format' });
...
// NOTE: Commented out for Vercel deployment where the frontend handles 404s
// app.use((req, res) => {
...
// --- Node Process Handlers (For local use only) ---
...
process.on('uncaughtException', (err) => {
  ...
  if (!process.env.VERCEL && client) { // Only attempt cleanup locally
    client.close().then(() => {
...
// --- Server Start/Export Logic (Vercel Compatibility) ---

// Check if running on Vercel platform
const isVercel = !!process.env.VERCEL;

if (isVercel) {
  // 1. Run the database connection setup once globally (during cold start)
  // The rest of the requests will reuse the 'db' variable inside withDatabaseErrorHandling
  connectToDatabase().catch(err => {
    console.error('Vercel cold start DB connection failed:', err);
    // Do NOT exit here, just log the error. Vercel expects an exported handler.
  });
  
  // 2. Export the Express app wrapped in serverless-http
  // This handler is called for every incoming request routed via vercel.json
