...
// Page Navigation - PENDING IS NOW LAST
const PAGES = {
  HOME: 'home',
  STATS: 'stats',
  HISTORY: 'history',
...
// ✅ NEW: Get current month date range (default)
...
const getDateRangePreset = (preset) => {
  ...
  switch (preset) {
    ...
    default:
      return getCurrentMonthRange(); // ✅ Default to this month
  }
};
...
const OfflineBanner = ({ isOnline }) => {
  ...
  return (
    <div className="bg-gradient-to-r from-red-500 to-red-600 text-white text-center py-3 px-4 text-sm font-medium sticky top-0 z-50 shadow-lg">
...
// ============================================
// STUDENT DETAIL MODAL - NEW COMPONENT
// ============================================

const StudentDetailModal = ({ student, onClose, dateRange = 'month' }) => {
  ...
  const fetchStudentDetails = useCallback(async () => {
    ...
    try {
      ...
      const studentId = student.userId || student._id || student.id;
      
      // Fetch attendance
      const attRes = await fetch(
        `${API_BASE}/admin/student/${studentId}/attendance?startDate=${range.start}&endDate=${range.end}`,
        { headers: { Authorization: `Bearer ${token}` } }
      );
      
      // Fetch gathas
      const gathaRes = await fetch(
        `${API_BASE}/admin/student/${studentId}/gathas?startDate=${range.start}&endDate=${range.end}`,
        { headers: { Authorization: `Bearer ${token}` } }
      );
      ...
      if (gathaRes.ok) {
        const data = await gathaRes.json();
        gathaData = Array.isArray(data) ? data : data.gathas || [];
      }

      // Group by date
      const groupedData = groupByDate(attendanceData, gathaData);
      
      setStudentData({
        ...student,
        groupedActivity: groupedData,
        totalAttendance: attendanceData.length,
        totalNewGathas: gathaData.filter(g => g.type === 'new').reduce((sum, g) => sum + (g.total_gatha || 0), 0),
        totalRevisions: gathaData.filter(g => g.type === 'revision').reduce((sum, g) => sum + (g.total_gatha || 0), 0),
      });
    } catch (err) {
  ...
  }, [student, selectedRange]);

  useEffect(() => {
    fetchStudentDetails();
  }, [fetchStudentDetails]);

  // Group attendance and gathas by date
  const groupByDate = (attendance, gathas) => {
    const grouped = {};

    // Add attendance
    attendance.forEach((att) => {
      const dateStr = formatLocalDateString(att.date);
      if (!grouped[dateStr]) {
        grouped[dateStr] = { date: dateStr, attendance: null, gathas: [] };
      }
      grouped[dateStr].attendance = att;
    });

    // Add gathas
    gathas.forEach((gatha) => {
      const dateStr = formatLocalDateString(gatha.date || gatha.created_at);
      if (!grouped[dateStr]) {
        grouped[dateStr] = { date: dateStr, attendance: null, gathas: [] };
      }
      grouped[dateStr].gathas.push(gatha);
    });
  ...
  return (
    <div className="fixed inset-0 bg-gray-900/80 backdrop-blur-sm flex items-center justify-center z-50 p-4" onClick={onClose}>
      <div
        className="bg-white rounded-3xl shadow-2xl w-full max-w-lg max-h-[90vh] overflow-hidden animate-in zoom-in duration-200"
        ...
        <div className="p-5 bg-gradient-to-r from-indigo-500 to-purple-600 text-white">
          <div className="flex items-center justify-between">
            <div className="flex items-center gap-3">
              ...
              <div>
                <h3 className="text-xl font-bold">{student.name || student.username}</h3>
                <p className="text-sm opacity-80">Student Activity</p>
              </div>
            </div>
          ...
          {isLoading ? (
            ...
            <div className="text-center py-12">
              ...
              <button
                ...
                className="mt-3 px-4 py-2 bg-indigo-100 text-indigo-700 rounded-xl font-medium text-sm"
              >
                Try Again
              </button>
            </div>
            ...
            <>
              ...
              <div className="p-4 grid grid-cols-3 gap-3">
                <div className="bg-green-50 rounded-xl p-3 text-center border border-green-200">
                  ...
                  <p className="text-2xl font-bold text-green-600">{studentData?.totalAttendance || 0}</p>
                  <p className="text-xs text-gray-500">Attendance</p>
                </div>
                <div className="bg-purple-50 rounded-xl p-3 text-center border border-purple-200">
                  <Plus className="w-6 h-6 text-purple-500 mx-auto mb-1" />
                  <p className="text-2xl font-bold text-purple-600">{studentData?.totalNewGathas || 0}</p>
                  <p className="text-xs text-gray-500">New Gathas</p>
                </div>
                <div className="bg-blue-50 rounded-xl p-3 text-center border border-blue-200">
                  <RefreshCw className="w-6 h-6 text-blue-500 mx-auto mb-1" />
                  <p className="text-2xl font-bold text-blue-600">{studentData?.totalRevisions || 0}</p>
              ...
              <div className="p-4">
                ...
                {(!studentData?.groupedActivity || studentData.groupedActivity.length === 0) ? (
                  ...
                  <div className="space-y-3">
                    {studentData.groupedActivity.map((day, index) => (
                      <div
                        key={day.date}
                        ...
                        <div className="bg-gray-100 px-4 py-2 flex items-center justify-between">
                          <div className="flex items-center gap-2">
                            ...
                            <span className="font-bold text-gray-700">
                              {formatDateIn(day.date, { weekday: 'short', day: 'numeric', month: 'short', year: 'numeric' })}
                            </span>
                          </div>
                          <div className="flex items-center gap-2">
                            {day.attendance && (
                            ...
                            {day.gathas.length > 0 && (
                              <span className="bg-purple-100 text-purple-700 text-xs px-2 py-0.5 rounded-full font-bold">
                                {day.gathas.length} Gatha{day.gathas.length > 1 ? 's' : ''}
                              </span>
                            )}
                          </div>
                        </div>
                        ...
                        <div className="p-3 space-y-2">
                          ...
                          {day.attendance && (
                            <div className="flex items-center gap-3 p-2 bg-green-50 rounded-lg border border-green-200">
                              ...
                              <div className="flex-1">
                                <p className="text-sm font-bold text-gray-800">Attendance Marked</p>
                                <p className="text-xs text-gray-500">
                                  Status: <PendingBadge status={day.attendance.status || 'approved'} size="small" />
                                </p>
                              </div>
                            </div>
                          )}
                          ...
                          {day.gathas.map((gatha, gIdx) => (
                            ...
                            >
                              <div className={`w-8 h-8 rounded-lg flex items-center justify-center ${
                                gatha.type === 'new' ? 'bg-purple-200' : 'bg-blue-200'
                              }`}>
                                {gatha.type === 'new' ? (
                              ...
                              <div className="flex-1">
                                <p className="text-sm font-bold text-gray-800">
                                  {gatha.sutra_name || gatha.sutraName}
                                </p>
                                <p className="text-xs text-gray-500">
                                  {gatha.type === 'new' ? 'New' : 'Revision'} • {gatha.which_gatha || gatha.whichGatha} • 
                                  <span className="font-bold"> {gatha.total_gatha || gatha.totalGatha} gathas</span>
                                </p>
                              </div>
                              <PendingBadge status={gatha.status || 'approved'} size="small" />
                            </div>
                          ))}

                          {/* No activity on this day */}
                          {!day.attendance && day.gathas.length === 0 && (
                            <p className="text-sm text-gray-400 text-center py-2">No activity recorded</p>
                          )}
                        </div>
                      </div>
                    ))}
...
const LeaderboardSection = ({ currentUserId, currentUserName, onStudentClick }) => {
  ...
  // Fetch leaderboard data - DEFAULT TO THIS MONTH
  const fetchLeaderboard = useCallback(async () => {
    ...
    try {
      ...
      let res = await fetch(
        `${API_BASE}/leaderboard?startDate=${range.start}&endDate=${range.end}`,
        { headers: { Authorization: `Bearer ${token}` } }
      );
      
      if (res.ok) {
        const data = await res.json();
        // ✅ Filter gathaLeaders to only include NEW gathas
        setLeaderboardData({
          attendanceLeaders: data.attendanceLeaders || [],
          gathaLeaders: (data.gathaLeaders || []).map(leader => ({
            ...leader,
            // Only count new gathas, not revisions
            totalGathas: leader.newGathas || leader.totalNewGathas || leader.totalGathas || 0,
          })),
        });
      } else {
        // Try analytics endpoint
        res = await fetch(
          `${API_BASE}/analytics/leaderboard?startDate=${range.start}&endDate=${range.end}`,
          { headers: { Authorization: `Bearer ${token}` } }
        );
        ...
        if (res.ok) {
          ...
          setLeaderboardData({
            ...
            gathaLeaders: (data.gathaLeaders || data.topGathas || []).map(leader => ({
              ...leader,
              totalGathas: leader.newGathas || leader.totalNewGathas || leader.totalGathas || 0,
            })),
          });
        } else {
          // Fallback: get all students and calculate locally
          ...
          if (studentsRes.ok) {
            ...
            // ✅ Sort by NEW gathas only (not revisions)
            const gathaLeaders = [...students]
              .sort((a, b) => (b.monthlyNewGathas || b.newGathas || 0) - (a.monthlyNewGathas || a.newGathas || 0))
              .slice(0, 5)
              .map((s, i) => ({
                ...s,
                rank: i + 1,
                totalGathas: s.monthlyNewGathas || s.newGathas || 0,
              }));
    ...
    <>
      <div className="bg-white rounded-2xl border-2 border-blue-200 shadow-sm overflow-hidden">
        ...
        <div className="flex p-2 bg-gray-100 gap-2">
          ...
          >
            <Plus className="w-4 h-4" /> New Gathas
          </button>
        </div>
            ...
            attendanceLeaders.length === 0 ? (
              ...
              <div className="space-y-2">
                {attendanceLeaders.slice(0, 5).map((user, index) => {
                  const odometerId = user.userId || user._id || user.id;
                  const isCurrentUser = odometerId === currentUserId || 
                  ...
                  return (
                    <button
                      key={odometerId || index}
                      onClick={() => handleStudentClick(user)}
                      className={`w-full flex items-center justify-between p-3 rounded-xl border-2 transition-all active:scale-[0.98] ${getRankBg(rank, isCurrentUser)}`}
                    >
                      <div className="flex items-center gap-3">
        ...
        <div className="px-4 pb-4">
          <div className="bg-blue-50 rounded-xl p-3 border border-blue-200">
            <p className="text-xs text-blue-700 text-center">
              ...
              Tap on a student to see their detailed activity. Leaderboard shows this month's data. 🌟
            </p>
          </div>
        </div>
      </div>
...
export default function StudentDashboard({ user, onLogout }) {
  ...
  const markAttendance = async () => {
    ...
    if (!navigator.onLine) {
      setGlobalError('You are offline. Please connect to the internet to mark attendance.');
      return;
    }
  ...
  const submitGatha = async (formData) => {
    // Check if online before making request
    if (!navigator.onLine) {
      setGlobalError('You are offline. Please connect to the internet to submit.');
      return;
    }
  ...
  const deletePendingGatha = async (id) => {
    ...
    // Check if online before making request
    if (!navigator.onLine) {
      setGlobalError('You are offline. Please connect to the internet to delete.');
      return;
    }
          ...
          <div className="grid grid-cols-2 gap-2">
            {HELPFUL_TIPS.map((tip, i) => (
              <div key={i} className="flex items-start gap-2 bg-white p-2 rounded-lg">
                <tip.icon className={`w-4 h-4 ${TIP_ICON_COLORS[tip.color]} flex-shrink-0 mt-0.5`} />
      ...
      <div className="bg-white rounded-2xl p-4 border-2 border-orange-200 shadow-sm">
        ...
        <div className="grid grid-cols-2 gap-3">
          <button
            ...
            disabled={todayAttendanceStatus !== 'not_marked' || isSubmitting || !isOnline}
          ...
          >
            {todayAttendanceStatus === 'approved' ? (
              ...
              <>
                <WifiOff className="w-10 h-10 mx-auto mb-2 text-gray-400" />
                <p className="font-bold text-gray-500">Offline</p>
                <p className="text-xs text-gray-400">Connect to mark</p>
              </>
            ) : (
              <>
          ...
          <button
            onClick={() => {
              if (!isOnline) {
                setGlobalError('You are offline. Please connect to the internet.');
                return;
              }
              setEditingGatha(null);
            ...
            disabled={!isOnline}
            className={`relative p-4 rounded-2xl shadow-lg active:scale-[0.97] transition-transform ${
              !isOnline
                ? 'bg-gray-100 border-2 border-gray-300'
          ...
          >
            {!isOnline ? (
              <>
                <WifiOff className="w-10 h-10 mx-auto mb-2 text-gray-400" />
                <p className="font-bold text-gray-500">Offline</p>
                <p className="text-xs text-gray-400">Connect to add</p>
              </>
            ) : (
              <>
            ...
            {isOnline && (todaysApprovedGathas.length + todaysPendingGathas.length > 0) && (
              <span className="absolute -top-2 -right-2 w-7 h-7 bg-green-500 rounded-full flex items-center justify-center text-sm font-bold shadow-lg text-white">
  ...
  return (
    <div className="min-h-screen bg-gradient-to-br from-orange-50 via-amber-50 to-yellow-50 pb-24">
      ...
      {/* Header */}
      <div className={`sticky ${isOnline ? 'top-0' : 'top-[44px]'} z-40 bg-white/95 backdrop-blur-sm shadow-md px-4 py-3`}>
        <div className="max-w-lg mx-auto flex items-center justify-between">
          <div className="flex items-center gap-3">
            ...
            <div>
              <h1 className="font-bold text-gray-800 text-sm leading-tight">શ્રી સોમચીન્તામણી વાસુપૂજ્યસ્વામી</h1>
              <p className="text-xs text-gray-500">જૈન પાઠશાળા</p>
            </div>
          </div>
          <div className="flex items-center gap-2">
            ...
            <button
              onClick={onLogout}
              className="flex items-center gap-2 bg-red-50 text-red-600 px-3 py-2 rounded-xl font-bold text-sm active:scale-[0.98] transition-transform border border-red-200"
            >
      ...
      <div className="max-w-lg mx-auto px-4 py-4">
        ...
        {/* Navigation Tabs - PENDING IS NOW LAST */}
