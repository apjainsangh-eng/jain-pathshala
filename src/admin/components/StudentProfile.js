...
export default function StudentProfile({ username, onBack, onAddAttendance, onAddGatha }) {
  ...
  if (!profile) {
    return (
      <div className="text-center py-10">
        ...
        <button onClick={onBack} className="mt-4 px-4 py-2 bg-indigo-500 text-white rounded-lg text-sm">Go Back</button>
      </div>
    );
  }
  ...
  const tabs = [
    ...
    { key: 'gatha', label: 'Gatha', icon: BookOpen },
  ...
  return (
    <div className="space-y-3 pb-4">
      ...
      <div className="bg-white rounded-2xl border-2 border-indigo-200 shadow-lg overflow-hidden">
        ...
        <div className="flex gap-2 p-3 bg-gradient-to-r from-indigo-50 to-purple-50 border-t border-indigo-100">
          <button onClick={() => onAddAttendance && onAddAttendance(username)}
            ...
            <Plus className="w-3.5 h-3.5" /> Attendance
          </button>
          <button onClick={() => onAddGatha && onAddGatha(username)}
            ...
            <Plus className="w-3.5 h-3.5" /> Gatha
          </button>
      ...
      <div className="grid grid-cols-4 gap-2">
        {[
          { label: 'Present', value: stats.totalPresent, color: 'text-green-600', bg: 'bg-green-50 border-green-200' },
          { label: 'Absent', value: stats.totalAbsent, color: 'text-red-600', bg: 'bg-red-50 border-red-200' },
          { label: 'Att %', value: stats.attendancePercent + '%', color: 'text-blue-600', bg: 'bg-blue-50 border-blue-200' },
          { label: 'Streak', value: stats.currentStreak, color: 'text-orange-600', bg: 'bg-orange-50 border-orange-200' },
        ].map((c, i) => (
      ...
      <div className="grid grid-cols-4 gap-2">
        {[
          { label: 'Gathas', value: stats.totalGathasSubmitted, color: 'text-purple-600', bg: 'bg-purple-50 border-purple-200' },
          { label: 'Pending', value: stats.totalPending, color: 'text-yellow-600', bg: 'bg-yellow-50 border-yellow-200' },
          { label: 'Score', value: stats.score, color: 'text-indigo-600', bg: 'bg-indigo-50 border-indigo-200' },
          { label: 'Rejected', value: stats.rejectedGathas, color: 'text-pink-600', bg: 'bg-pink-50 border-pink-200' },
        ].map((c, i) => (
          <div key={i} className={`${c.bg} border rounded-xl p-2 text-center`}>
            <p className={`text-lg font-bold ${c.color}`}>{c.value}</p>
            <p className="text-[10px] text-gray-500 font-semibold">{c.label}</p>
      ...
      <div className="flex gap-1 bg-gray-100 p-1 rounded-xl overflow-x-auto">
        {tabs.map(t => (
          <button key={t.key} onClick={() => setActiveTab(t.key)}
            className={`flex items-center gap-1 flex-1 py-2 px-2 rounded-lg text-xs font-bold whitespace-nowrap transition-all ${activeTab === t.key ? 'bg-white text-indigo-700 shadow' : 'text-gray-500'}`}>
      ...
      {activeTab === 'overview' && (
        <div className="space-y-3">
          ...
          <div className="bg-white rounded-xl border-2 border-gray-200 p-3 space-y-2">
            <div className="flex justify-between text-xs"><span className="text-gray-500">Join Date</span><span className="font-bold text-gray-700">{formatDate(student.joinDate)}</span></div>
            <div className="flex justify-between text-xs"><span className="text-gray-500">Last Attendance</span><span className="font-bold text-gray-700">{formatDate(student.lastAttendanceDate)}</span></div>
            <div className="flex justify-between text-xs"><span className="text-gray-500">Last Gatha</span><span className="font-bold text-gray-700">{formatDate(student.lastGathaDate)}</span></div>
            <div className="flex justify-between text-xs"><span className="text-gray-500">Max Streak</span><span className="font-bold text-orange-600">{stats.maxStreak} days</span></div>
            <div className="flex justify-between text-xs"><span className="text-gray-500">New Gathas</span><span className="font-bold text-purple-600">{stats.totalNewGathas}</span></div>
            <div className="flex justify-between text-xs"><span className="text-gray-500">Revision Gathas</span><span className="font-bold text-blue-600">{stats.totalRevisionGathas}</span></div>
          </div>
          ...
          {recentActivity && recentActivity.length > 0 && (
            <div className="bg-white rounded-xl border-2 border-gray-200 p-3">
              <p className="text-sm font-bold text-gray-700 mb-2">Recent Activity</p>
                  ...
                  <div key={i} className="flex items-center gap-2 bg-gray-50 rounded-lg p-2">
                    ...
                    <div className="flex-1 min-w-0">
                      <p className="text-xs font-semibold text-gray-700 truncate">
                        {act.type === 'attendance' ? 'Present' : `${act.sutraName || 'Gatha'} (${act.totalGatha})`}
                      </p>
      ...
      {activeTab === 'attendance' && (
        <div className="space-y-2">
          <div className="bg-white rounded-xl border-2 border-gray-200 p-3">
            <p className="text-sm font-bold text-gray-700 mb-2">Attendance History ({(attendanceHistory||[]).length} days)</p>
              ...
              {(attendanceHistory || []).map((a, i) => (
                <div key={i} className="flex items-center justify-between bg-green-50 rounded-lg p-2.5 border border-green-100">
                  <div>
                    ...
                    <p className="text-[10px] text-gray-400">Marked by: {a.markedBy} · Approved: {a.approvedBy}</p>
                  </div>
                  <span className="text-xs bg-green-500 text-white px-2 py-0.5 rounded-full font-bold">Present</span>
                </div>
              ))}
              {(attendanceHistory || []).length === 0 && <p className="text-center text-gray-400 text-sm py-4">No attendance records</p>}
            </div>
          </div>
          {(pendingAttendanceHistory || []).filter(p => p.status === 'pending').length > 0 && (
            <div className="bg-white rounded-xl border-2 border-yellow-200 p-3">
              <p className="text-sm font-bold text-yellow-700 mb-2">Pending Attendance</p>
              {pendingAttendanceHistory.filter(p => p.status === 'pending').map((a, i) => (
                <div key={i} className="flex items-center justify-between bg-yellow-50 rounded-lg p-2.5 mb-1">
                  <p className="text-xs font-bold text-gray-700">{formatDate(a.date)}</p>
                  <span className="text-xs bg-yellow-400 text-yellow-900 px-2 py-0.5 rounded-full font-bold">Pending</span>
                </div>
              ))}
            </div>
          )}
      ...
      {activeTab === 'gatha' && (
        <div className="space-y-2">
          <div className="bg-white rounded-xl border-2 border-gray-200 p-3">
            <p className="text-sm font-bold text-gray-700 mb-2">Gatha History ({(gathaHistory || []).length} entries)</p>
            <div className="space-y-1 max-h-96 overflow-y-auto">
              {(gathaHistory || []).map((g, i) => (
                <div key={i} className="bg-purple-50 rounded-lg p-2.5 border border-purple-100">
                  <div className="flex items-center justify-between">
                    <div>
                      <p className="text-xs font-bold text-gray-700">{g.sutraName || 'Gatha'}</p>
                      <p className="text-[10px] text-gray-500">{g.whichGatha} · Count: {g.totalGatha}</p>
                      <p className="text-[10px] text-gray-400">{formatDate(g.date)} · By: {g.addedBy}</p>
                    </div>
                    <div className="text-right">
                      <span className={`text-xs px-2 py-0.5 rounded-full font-bold ${g.type === 'new' ? 'bg-purple-500 text-white' : 'bg-blue-100 text-blue-600'}`}>
                        {g.type === 'new' ? 'New' : 'Revision'}
                      </span>
              ...
              {(gathaHistory || []).length === 0 && <p className="text-center text-gray-400 text-sm py-4">No gatha records</p>}
            </div>
          </div>
        </div>
      )}
      ...
      {activeTab === 'analytics' && (
        <div className="space-y-3">
          {/* Monthly chart */}
          <div className="bg-white rounded-xl border-2 border-gray-200 p-3">
            <p className="text-sm font-bold text-gray-700 mb-3">Monthly Trends ({now.getFullYear()})</p>
            ...
            <div className="flex gap-4 mt-2 justify-center">
              <span className="flex items-center gap-1 text-[10px]"><span className="w-3 h-3 rounded bg-green-400" /> Attendance</span>
              <span className="flex items-center gap-1 text-[10px]"><span className="w-3 h-3 rounded bg-purple-400" /> New Gathas</span>
            </div>
          </div>
              ...
              return (
                <>
                  <div className="bg-green-50 border border-green-200 rounded-xl p-3 text-center">
                    <p className="text-[10px] text-gray-500 font-semibold">Best Month</p>
                    <p className="text-sm font-bold text-green-600">{best ? mo[best.idx] : '-'}</p>
                    <p className="text-xs text-gray-500">{best?.attendance || 0} days</p>
                  </div>
                  <div className="bg-red-50 border border-red-200 rounded-xl p-3 text-center">
                    <p className="text-[10px] text-gray-500 font-semibold">Weakest Month</p>
                    <p className="text-sm font-bold text-red-600">{worst ? mo[worst.idx] : '-'}</p>
                    <p className="text-xs text-gray-500">{worst?.attendance || 0} days</p>
                  </div>
                </>
              );
            })()}
      ...
      {activeTab === 'notes' && (
        <div className="space-y-3">
          <div className="bg-white rounded-xl border-2 border-gray-200 p-3">
            <p className="text-sm font-bold text-gray-700 mb-2">Admin Notes</p>
            <div className="flex gap-2">
              ...
              <button onClick={handleAddNote} disabled={addingNote || !noteText.trim()}
                ...
                {addingNote ? <RefreshCw className="w-4 h-4 animate-spin" /> : 'Add'}
              </button>
            </div>
          </div>
          <div className="space-y-2">
            {(notes || []).map((note, i) => (
              <div key={i} className="bg-yellow-50 border-l-4 border-yellow-400 rounded-r-xl p-3">
                ...
                <p className="text-[10px] text-gray-400 mt-1">By {note.createdBy} · {formatDate(note.createdAt)}</p>
              </div>
            ))}
